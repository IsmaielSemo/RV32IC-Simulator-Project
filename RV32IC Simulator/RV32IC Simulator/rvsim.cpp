...
bool isCompressed(unsigned int opcode){
    opcode = opcode & 0x0000003; //last 2 bits
    if (opcode == 0x0 || opcode == 0x1) {
        return true;  // It is a compressed instruction
    } else {
        return false; // It is not a compressed instruction
    }
}
...
void instDecExec(unsigned int instWord)
{
    ...
    unsigned int funct4; //for compressed instruction
    ...
    bool compressedStatus = isCompressed(opcode);
            ...
            switch (funct3) {
                case 0:
                    ...
                    reg[rd] = (memory[reg[rs1] + (signed) I_imm]) |
                              (((memory[reg[rs1] + (signed) I_imm] & 0x80) == 0x80) ? 0xFFFFFF00 : 0);
                    break;
                case 1:
                    cout << "\tLH\tx" << rd << ", x" << rs1 << ", " << hex << "0x" << (int) I_imm << "\n";
                    reg[rd] = (memory[reg[rs1] + (signed) I_imm]) | (memory[reg[rs1] + (signed) I_imm + 1] << 8) |
                              (((memory[reg[rs1] + (signed) I_imm + 1] & 0x80) == 0x80) ? 0xFFFF0000 : 0);
                    break;
                case 2:
                    cout << "\tLW\tx" << rd << ", x" << rs1 << ", " << hex << "0x" << (int) I_imm << "\n";
                    reg[rd] = (memory[reg[rs1] + (signed) I_imm]) | (memory[reg[rs1] + (signed) I_imm + 1] << 8) |
                              (memory[reg[rs1] + (signed) I_imm + 2] << 16) |
                              (memory[reg[rs1] + (signed) I_imm + 3] << 24);
                    break;
                case 4:
